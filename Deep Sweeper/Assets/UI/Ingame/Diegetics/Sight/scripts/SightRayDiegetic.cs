...
{
    public class SightRayDiegetic : Diegetic
    {
        ...
        private void OnPrimaryOperationClick() {
            if (CursorViewer.Instance.IsDisplayed) return;
            
            PrimaryTriggerEvent?.Invoke(TargetType, Target, false);
        ...
        private TargetInfo SelectMine(GameObject mine) {
            ...
            if (mineInfo.IsValueable) {
                ...
                CrosshairManager.Instance.ActiveCrosshair.Lock();
                return mineInfo;
            }
            else return null;
        }
