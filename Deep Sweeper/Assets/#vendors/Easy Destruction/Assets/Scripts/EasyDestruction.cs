...
public class ExplodedSurfaces
{
    ...
    //amount of particles the mesh breaks into
...
public class EasyDestruction : MonoBehaviour {
    ...
    public void explode(Transform transform)
    {
        ...
        for (int submesh = 0; submesh < M.subMeshCount; submesh++)
        {
            ...
            for (int i = 0; i < explodedSurfaces.particlesCount; i += 3)
            {
                ...
                newSurface.transform.localScale = Vector3.one * explodedSurfaces.particleSize;
                ...
                Rigidbody surfaceRigidBody = newSurface.AddComponent<Rigidbody>();
                surfaceRigidBody.useGravity = false;
                surfaceRigidBody.AddExplosionForce(UnityEngine.Random.Range(explosionSettings.explosionMin, explosionSettings.explosionMax), explosionPos, explosionSettings.explosionRadius);

                if (explodedSurfaces.ShouldBeRemovedInTime)
    ...
    public void Start()
    {
        ...
        if (transform.GetComponent<SkinnedMeshRenderer>() != null)
            this.skinned = transform.GetComponent<SkinnedMeshRenderer>();

        if (transform.GetComponent<Collider>() != null)
            this.col = transform.GetComponent<Collider>();
